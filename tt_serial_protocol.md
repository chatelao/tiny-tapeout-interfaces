...
## General Principles
- **Line-based**: Every request from the host must be a single line ending with
a newline character (`\n`).
...
To perform a single transaction (update inputs and read outputs), the host sends
 a 6-character hexadecimal string or a semicolon-separated string.
...
Short Format, elements omitted at the end of the line are kept to the same value
 "as is":
